...
class SearchRequest(BaseModel):
    """Запрос поиска релевантных чанков по вектору."""

    query_vector: List[float] = Field(..., min_length=1, description="Вектор запроса")
    limit: int = Field(default=5, ge=1, le=100, description="Максимальное количество результатов")


class DocumentResponse(BaseModel):
    """Документ (чанк) в ответе поиска."""
